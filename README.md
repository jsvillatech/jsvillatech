## Hey there 👋, I'm [J. Steven Delgado V.](https://github.com/jsvillatech)

[![Linkedin Badge](https://img.shields.io/badge/-LinkedIn-0e76a8?style=flat-square&logo=Linkedin&logoColor=white)](https://linkedin.com/in/jsdelvil)
[![Kaggle Badge](https://img.shields.io/badge/-Kaggle-20beff?style=flat-square&logo=kaggle&logoColor=white)](https://kaggle.com/jhoansteven)
[![Medium Badge](https://img.shields.io/badge/-Medium-black?style=flat-square&logo=medium&logoColor=white)](https://medium.com/@jst3ch)
[![Gmail Badge](https://img.shields.io/badge/-Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:jst3ch@gmail.com)
[![Resume Badge](https://img.shields.io/badge/-Resume-555?style=flat-square&logo=file&logoColor=white)](https://drive.google.com/file/d/1pLo-dGAWK5gX8xdObdwJ7k4TsvwlgU6x/view?usp=share_link)

---

### 👋 Glad to see you here!

<p align="left"> 
  <img src="https://komarev.com/ghpvc/?username=jsvillatech&label=Profile%20views&color=0e75b6&style=flat" alt="jsvillatech" /> 
</p>

<p align="justify">
I’m a <b>Data Scientist</b> & <b>ML Engineer</b> passionate about designing, training, and deploying 
<b>machine learning</b> and <b>generative AI</b> systems that deliver real-world impact.  
With over <b>5 years of experience</b>, I specialize in <b>LLMs</b>, <b>production-ready ML pipelines</b>, 
and <b>AI automation</b> using <b>AWS</b>, <b>GCP</b>, and <b>Docker</b>.
</p>

---

### 🧠 About Me

- 🤖 I design and deploy **LLM-powered systems** (OpenAI, Vertex AI, Dialogflow CX)  
- ⚙️ Experienced in **MLOps**, model serving, and **CI/CD for ML**  
- 🧩 Focused on **machine learning**, **NLP**, and **applied generative AI**  
- 🧪 Passionate about **AI research** — co-author of **2 IEEE papers** on **machine learning and cybersecurity**, and currently working on a paper exploring **Support Vector Regression subsampling algorithms** to improve **computational complexity on big datasets**  
- ☁️ Skilled with **AWS** and **GCP (Vertex AI, Lambda, Dataflow, BQML)**  
- 🎓 MSc in Data Science & Postgrad in Finance — Universidad Icesi  
- 💬 Ask me about: **MLOps, LLMs, Model Deployment, Generative AI, and Vertex AI**

---

### 🧰 Tech Stack

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="Python" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/tensorflow/tensorflow-original.svg" alt="TensorFlow" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pytorch/pytorch-original.svg" alt="PyTorch" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" alt="Docker" width="50" height="50"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/9/93/Amazon_Web_Services_Logo.svg"
       alt="AWS" width="60" height="40"
       style="background-color:#ffffff; padding:4px; border-radius:6px;"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/googlecloud/googlecloud-original.svg" alt="GCP" width="50" height="50"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="Git" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/jupyter/jupyter-original.svg" alt="Jupyter" width="45" height="45"/>
    <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/bash/bash-original.svg" 
       alt="Bash" width="45" height="45" 
       style="filter: invert(1) hue-rotate(180deg);"/>
</p>

---

### 📂 Featured Projects

| Project | Description | Tech |
|----------|--------------|------|
| [💬 LLM RAG Chatbot](#) | Built a secure LLM-based chatbot integrating OpenAI API and AWS RDS for contextual Q&A | Python • LangChain • AWS • Docker |
| [📈 Product Recommendation Model](#) | Developed an implicit matrix factorization model (NDCG ↑90%) with Vertex AI & BQML | Python • Vertex AI • BigQuery ML |
| [📊 Revenue Forecasting](#) | Forecasted monthly revenue using Prophet, improving accuracy by 15% | Python • Prophet • Pandas • Plotly |
| [🧩 SVRSubsample](https://github.com/jsvillatech/SVRSubsample) | Implemented a subsampling algorithm to scale Support Vector Regression efficiently | Python • Scikit-Learn • Numpy |
| [🛡️ Secure Learning for Android Malware Detection](https://github.com/jsvillatech/SL_Android_Malware_-PDG-) | Research project applying **secure learning** techniques to defend ML models against **adversarial attacks** in Android malware classification | Python • Scikit-Learn • Pandas • Ggplot |

---

### 📚 Research & Publications

- **Features to Detect Android Malware** – *IEEE Colombian Conference on Communications and Computing (COLCOM), 2018*  
  [🔗 View Paper](https://ieeexplore.ieee.org/document/8466715/)
  
- **Exploring Machine Learning to Analyze Parkinson’s Disease Patients** – *IEEE International Conference on Semantics, Knowledge and Grids (SKG), 2018*  
  [🔗 View Paper](https://ieeexplore.ieee.org/document/8703872)
  
- **In Progress:** Research paper on **Support Vector Regression subsampling algorithms** to improve **computational complexity on big datasets**.

---

### 📊 GitHub Stats

<details>
  <summary><b>⚡ Stats & Top Languages</b></summary>
  <br/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=jsvillatech&show_icons=true&hide_border=true&count_private=true&include_all_commits=true&theme=tokyonight"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=jsvillatech&layout=compact&theme=tokyonight"/>
</details>

<details>
  <summary><b>🔥 GitHub Streak</b></summary>
  <br/>
  <img height="180em" src="https://github-readme-streak-stats.herokuapp.com/?user=jsvillatech&theme=tokyonight&hide_border=true"/>
</details>

---

<div align="center">

### 🎥 Content & Community
🎬 *Building the bridge between research, data science, and real-world MLOps.*  

🧠 I love sharing insights about Data Science, Machine Learning, and MLOps.  
📺 Coming soon: tutorials, hands-on projects, and breakdowns on my future [YouTube Channel](#)  

---

### ☕ Support My Work
If my projects or future tutorials help you, you’ll soon be able to support my work here!  
<a href="#" target="_blank">
  <img src="https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png" alt="Buy Me A Coffee" height="45" width="180">
</a>

---

</div>

<div align="center">

### 🚀 Let’s Connect  
💡 Interested in collaborating on AI, MLOps, or applied ML research?  
📬 Reach out via [LinkedIn](https://linkedin.com/in/jsdelvil) or [Email](mailto:jst3ch@gmail.com)

---


</div>
